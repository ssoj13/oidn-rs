...
fn progress_callback_fires_per_tile() {
    ...
    assert!(
        (last - 1.0).abs() < 1e-6,
        "progress did not finish at 1.0, got {last}"
    );
...
fn user_weights_blob_bypasses_registry() {
    ...
    let color = synth_color(w, h);
    let mut filter = RtFilter::builder(&device, PathBuf::from("/nonexistent"))
        .hdr(true)
...
fn input_scale_explicit_vs_unscaled() {
    ...
    let mut a = RtFilter::builder(&device, &dir)
        .hdr(true)
        .quality(Quality::Balanced)
        .input_scale(Some(1.0))
        .build();
    ...
    let mut b = RtFilter::builder(&device, &dir)
        .hdr(true)
        .quality(Quality::Balanced)
    ...
    assert!(
        rmse_diff > 1e-4,
        "explicit input_scale appears to have no effect"
    );
}
...
fn memory_budget_forces_more_tiles() {
    ...
    assert!(
        plan_8mb.tile_w * plan_8mb.tile_h <= plan_no_budget.tile_w * plan_no_budget.tile_h,
