...
fn all_shipped_models_load_and_forward() {
    ...
    for entry in std::fs::read_dir(&dir).unwrap() {
        ...
        let tensors =
            oidn_tza::parse(&bytes).unwrap_or_else(|e| panic!("parse failed for {stem}: {e}"));
        ...
        if stem.ends_with("_large") {
            ...
            let unet = UNet::new(in_ch, out_ch, variant, &device);
    ...
    assert!(
        by_variant[2] >= 3,
        "expected at least 3 _large variants, got {}",
        by_variant[2]
    );
    assert!(
        by_variant[1] >= 6,
        "expected at least 6 _small variants, got {}",
        by_variant[1]
    );
}
