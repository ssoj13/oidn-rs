...
fn run_rgb_f32_round_trip() {
    ...
    for (i, v) in src.iter_mut().enumerate() { *v = (i as f32) * 0.1; }
...
#[test]
fn rgb_f32_round_trip() { run_rgb_f32_round_trip(); }

#[test]
fn rgb_f16_round_trip() {
    let (w, h) = (4usize, 3usize);
    let mut src = vec![f16::ZERO; w * h * 3];
    for (i, v) in src.iter_mut().enumerate() { *v = f16::from_f32(i as f32 * 0.1); }
    ...
    for (a, b) in dst.iter().zip(src.iter()) {
        assert!((a.to_f32() - b.to_f32()).abs() < 1e-3, "f16 round-trip drift");
    }
}
