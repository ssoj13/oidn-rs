...
pub fn plan(
    ...
    while (tile_h * tile_w) > max_tile_pixels {
        if tile_h > min_tile_h && tile_h > tile_w {
            let new_h = ceil_div(height + (2 * tile_overlap + pad_h) * tile_count_h, tile_count_h + 1);
            ...
            tile_count_h = std::cmp::max(
                ceil_div(height - (2 * tile_overlap + pad_h), tile_h - (2 * tile_overlap + pad_h)),
                1,
            );
        } else if tile_w > min_tile_w {
            let new_w = ceil_div(width + (2 * tile_overlap + pad_w) * tile_count_w, tile_count_w + 1);
            ...
            tile_count_w = std::cmp::max(
                ceil_div(width - (2 * tile_overlap + pad_w), tile_w - (2 * tile_overlap + pad_w)),
                1,
            );
        } else {
    ...
    for i in 0..tile_count_h {
        ...
        let overlap_bottom = if i < tile_count_h - 1   { tile_overlap + pad_h } else { 0 };
        ...
        for j in 0..tile_count_w {
            ...
            let overlap_right = if j < tile_count_w - 1   { tile_overlap + pad_w } else { 0 };
            ...
            jobs.push(TileJob {
                input: Rect { x, y, w: tile_w1, h: tile_h1 },
                output_dst: Rect {
                    x: x + overlap_left,
                    y: y + overlap_top,
                    w: tile_w2,
    ...
    TilePlan { tile_w, tile_h, overlap: tile_overlap, pad_w, pad_h, jobs }
}

/// Convenience: total pixel count covered by all output rectangles.
pub fn total_output_pixels(plan: &TilePlan) -> i64 {
    plan.jobs.iter().map(|j| (j.output_dst.w as i64) * (j.output_dst.h as i64)).sum()
}
