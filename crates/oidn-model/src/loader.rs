...
pub enum LoadError {
    ...
    BadLayout { name: String, expected: &'static str, got: Layout },

    #[error("tensor {name:?} expected shape {expected:?}, got {got:?}")]
    ShapeMismatch { name: String, expected: Vec<usize>, got: Vec<u32> },
}

fn fetch<'a>(map: &'a TensorMap, name: &str) -> Result<&'a TzaTensor, LoadError> {
    map.get(name).ok_or_else(|| LoadError::MissingTensor(name.to_owned()))
}

/// Build a 4-D Burn `Param` from an `oihw` TZA tensor.
fn into_param4<B: Backend>(
    name: &str,
    src: &TzaTensor,
    expected_shape: [usize; 4],
    device: &B::Device,
) -> Result<Param<Tensor<B, 4>>, LoadError> {
    ...
    if src.desc.dims.len() != 4
        || (0..4).any(|i| src.desc.dims[i] as usize != expected_shape[i])
    {
    ...
    let tensor = Tensor::<B, 4>::from_data(TensorData::new(data, expected_shape), device);
    Ok(Param::initialized(ParamId::new(), tensor))
}

/// Build a 1-D Burn `Param` (bias) from an `x` TZA tensor.
fn into_param1<B: Backend>(
    name: &str,
    src: &TzaTensor,
    expected_len: usize,
    device: &B::Device,
) -> Result<Param<Tensor<B, 1>>, LoadError> {
    ...
    let tensor = Tensor::<B, 1>::from_data(TensorData::new(data, [expected_len]), device);
    Ok(Param::initialized(ParamId::new(), tensor))
}

/// Overwrite `conv`'s weight and bias from `<layer>.weight` / `<layer>.bias`.
fn load_conv<B: Backend>(
    mut conv: Conv2d<B>,
    layer: &str,
    map: &TensorMap,
    device: &B::Device,
) -> Result<Conv2d<B>, LoadError> {
    ...
    conv.weight = into_param4::<B>(&w_name, w_src, w_shape, device)?;
    conv.bias = Some(into_param1::<B>(&b_name, b_src, b_len, device)?);
    Ok(conv)
}

/// Load all weights from a TZA `TensorMap` into the given U-Net.
pub fn load_tza<B: Backend>(
    unet: UNet<B>,
    map: &TensorMap,
    device: &B::Device,
) -> Result<UNet<B>, LoadError> {
    Ok(UNet {
...
pub fn load_tza_large<B: Backend>(
    unet: UNetLarge<B>,
    map: &TensorMap,
    device: &B::Device,
) -> Result<UNetLarge<B>, LoadError> {
    Ok(UNetLarge {
