...
fn load_rt_hdr_into_unet() {
    ...
    for x in &v { assert!(x.is_finite(), "non-finite value"); }
    assert!(mean_abs > 0.0, "output is identically zero — load_tza did not populate weights");
}

#[test]
fn load_rt_hdr_alb_nrm_into_unet_9ch() {
    let Some(path) = weights_path("rt_hdr_alb_nrm") else { return; };
    ...
    // 9-channel input.
    let input = Tensor::<B, 4>::ones([1, 9, 32, 32], &device) * 0.25;
    let out = unet.forward(input).into_data();
    let v: Vec<f32> = out.convert::<f32>().to_vec().unwrap();
