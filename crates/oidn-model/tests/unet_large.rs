...
fn unet_large_base_forward_shape() {
    ...
    let unet = oidn_model::UNetLarge::<B>::new(3, 3, &device);
...
fn unet_large_xl_forward_shape() {
    type B = NdArray<f32>;
    let device = Default::default();
    let unet = oidn_model::UNetLarge::<B>::new_xl(3, 3, &device);
    let input = Tensor::<B, 4>::zeros([1, 3, 96, 96], &device);
    let out = unet.forward(input);
...
fn load_rt_alb_large() {
    let Some(path) = weights_path("rt_alb_large") else { return; };
    type B = NdArray<f32>;
    let device = Default::default();

    let bytes = std::fs::read(&path).unwrap();
    let tensors = oidn_tza::parse(&bytes).unwrap();
    assert_eq!(oidn_model::Variant::from_tensor_names(tensors.keys()), oidn_model::Variant::Large);
...
fn load_rt_nrm_large() {
    let Some(path) = weights_path("rt_nrm_large") else { return; };
    type B = NdArray<f32>;
    let device = Default::default();
...
fn load_rt_hdr_calb_cnrm_large_9ch() {
    let Some(path) = weights_path("rt_hdr_calb_cnrm_large") else { return; };
    type B = NdArray<f32>;
    let device = Default::default();
    ...
    let unet = oidn_model::UNetLarge::<B>::new(9, 3, &device);
    let unet = oidn_model::load_tza_large(unet, &tensors, &device).unwrap();
    let input = Tensor::<B, 4>::ones([1, 9, 64, 64], &device) * 0.25;
    let out = unet.forward(input).into_data();
    let v: Vec<f32> = out.convert::<f32>().to_vec().unwrap();
    for x in &v { assert!(x.is_finite()); }
}
